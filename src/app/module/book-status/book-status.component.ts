...
import { ACTION, APPOINTMENT_DELETED, APPOINTMENT_NOT_FOUND, appointmentListKey } from '../../enum/localStorage-enum';
...
export class BookStatusComponent implements OnInit {
  appointmentList!: Observable<formData[]>;
  appointmentListData!: formData[];
  id!: string;
  transformedDate!: string;
  currentAppointmentListData!: formData[];
  pastAppointmentListData!: formData[];
  ...
  constructor(private store: Store<AppState>, private route: ActivatedRoute, private datePipe: DatePipe,
  ...
  deleteAppointment(id: number): void {
    ...
    //   // if (parsedFormData) {
    //     // const dataToDelete = parsedFormData.findIndex((value: { id: number }) => value.id === id);
    //     // if (dataToDelete !== -1) {
    //       // parsedFormData.splice(dataToDelete, 1);
    //       // this.restrauntService.deleteAppointment(parsedFormData);
